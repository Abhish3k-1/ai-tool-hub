...
import { Globe, Search, ExternalLink, Loader2, Building2 } from 'lucide-react';
...
export default function AIResearchPage() {
    ...
    const handleSearch = async (e?: React.FormEvent) => {
        ...
        if (!query.trim()) {
            setError('Please enter a topic or keyword to research.');
            return;
        }
        ...
        try {
            const response = await fetch('/api/tools/ai-research', {
                method: 'POST',
            ...
            trackUsage('research', user?.email);
        } catch (err: unknown) {
        ...
        <ProtectedRoute>
            <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 page-enter">
                <div className="mb-8 flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">AI Research</h1>
                        <p className="text-slate-600">Deeply analyze any topic with real-time web awareness.</p>
                    </div>
                </div>
                ...
                <div className="space-y-8">
                        ...
                        <CardContent className="pt-6">
                            ...
                            >
                                <div className="relative flex-grow">
                                    ...
                                    <Input
                                        type="text"
                                        placeholder="Topic, technology, or company name..."
                                        value={query}
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-end px-1">
                            ...
                            {hasSearched && !loading && (
                                <span className="text-sm text-slate-500">Found {results.length} relevant sources</span>
                            )}
                        </div>
                        ...
                        {loading && (
                            <Card className="flex min-h-[300px] flex-col items-center justify-center space-y-4 border-dashed border-2 border-sky-200 bg-sky-50/70 text-sky-700 shadow-none">
                                ...
                                <div className="text-center">
                                    <p className="font-medium animate-pulse">Scanning the web for deep insights...</p>
                        ...
                        {!hasSearched && !loading && (
                            <Card className="relative flex min-h-[300px] items-center justify-center border-dashed border-2 border-slate-200 bg-slate-50 shadow-none">
                                <div className="text-center p-8 max-w-sm">
                                    ...
                                    <h4 className="mb-2 font-medium text-slate-900">Start your research</h4>
                                    <p className="text-sm text-slate-500">
                                        Enter a topic or keyword above and hit search to gather insights via Firecrawl AI.
                                    </p>
                                </div>
                            </Card>
                        )}
                        ...
                        {hasSearched && !loading && results.length === 0 && !error && (
                            <Card className="flex min-h-[200px] items-center justify-center border border-slate-200 bg-slate-50">
                                <p className="font-medium text-slate-500">No insights found for &quot;{query}&quot;. Try a broader term.</p>
                            </Card>
                        )}
